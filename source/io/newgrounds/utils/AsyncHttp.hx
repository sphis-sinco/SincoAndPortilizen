...
class AsyncNGCall
{
	...
	static public function send(core:NGLite, data:String, onData:String->Void, onError:String->Void, onStatus:Int->Void)
	{
...
class AsyncHttp
{
	...
	static public function send(path:String, data:String, onData:String->Void, onError:String->Void, ?onStatus:Int->Void)
	{
		// core.logVerbose('sending: $data');
	...
	static function sendSync(path:String, data:String, onData:String->Void, onError:String->Void, ?onStatus:Int->Void)
	{
		trace('Sync send');
	...
	static function sendAsync(path:String, data:String, onData:(String) -> Void, onError:(String) -> Void, ?onStatus:(Int) -> Void,
	...
	static function sendThreaded():Void
	{
		while (true)
		{
			...
			sendSync(data.path, data.args, function(reply)
			{
				data.source.sendMessage({key: data.key, data: reply});
			}, function(error)
			{
				data.source.sendMessage({key: data.key, error: error});
			}, function(status)
			{
				data.source.sendMessage({key: data.key, status: status});
			});
		}
	}
	#end
